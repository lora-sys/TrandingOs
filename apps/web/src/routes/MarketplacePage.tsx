  ...
  return (
    <section className="pageStack">
      ...
      <Card className="heroPanel marketHero">
        <Card.Header className="panelTitle"><Boxes size={16} /> OS Marketplace <Chip size="sm" color="accent" variant="primary">local-first</Chip></Card.Header>
        <div className="marketHeroGrid">
          ...
          <Button variant="default" onClick={() => seed.mutate()} disabled={seed.isPending}><RefreshCw size={16} /> Seed Catalog</Button>
        </div>
      </Card>
      <Card className="controlPanel heroPanel">
        ...
        <form className="workspaceForm" onSubmit={(event) => { event.preventDefault(); void form.handleSubmit(); }}>
          ...
          <Button type="submit" variant="default" disabled={register.isPending}>Register MCP</Button>
          <Button variant="secondary" onClick={() => discover.mutate("")} disabled={discover.isPending}>Discover</Button>
        </form>
      </Card>
