  ...
  return (
    <section className="pageStack">
      ...
      <Card className="controlPanel heroPanel">
        <Card.Header className="panelTitle"><FolderKanban size={16} /> Workspace Manager <Chip size="sm" variant="primary">OS domain</Chip></Card.Header>
        <form className="workspaceForm" onSubmit={(event) => { event.preventDefault(); void form.handleSubmit(); }}>
          ...
          <Button type="submit" variant="default" disabled={mutation.isPending}><Plus size={16} /> Save</Button>
          <Button variant="secondary" onClick={() => bootstrap.mutate()} disabled={bootstrap.isPending}>Bootstrap OS</Button>
        </form>
      </Card>
